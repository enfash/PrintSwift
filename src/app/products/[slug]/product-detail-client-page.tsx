...
export default function ProductDetailClientPage({ product, faqs, categories, allProducts }: { product: any, faqs: any[], categories: any[], allProducts: any[] }) {
  ...
  useEffect(() => {
    if (product) {
        setSelectedImage(product.mainImageIndex || 0);
        ...
        const minQty = product.pricing?.tiers?.filter((t: any) => t.minQty > 0).reduce((min: number, tier: any) => Math.min(min, tier.minQty), Infinity) || 1;
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="space-y-4 md:sticky top-24 self-start">
              <div className="aspect-square relative rounded-lg border overflow-hidden">
                  <Image
                      src={product.blurDataUrls?.[selectedImage] || mainImageUrl}
                      placeholder={product.blurDataUrls?.[selectedImage] ? 'blur' : 'empty'}
                      blurDataURL={product.blurDataUrls?.[selectedImage]}
                      alt={product.name}
                      fill
